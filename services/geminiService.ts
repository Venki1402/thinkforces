...
if (!apiKey) {
  console.error("GEMINI_API_KEY is not set. Please create a .env.local file with GEMINI_API_KEY=your_key");
}
...
export const sendMessageStream = async function* (
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isRateLimitError(error) && attempt < maxRetries) {
        ...
        console.warn(`Rate limit hit (429). Retrying in ${backoffMs / 1000}s... (attempt ${attempt + 1}/${maxRetries + 1})`);
        
        // Yield a message to inform the user
        yield `\n[Rate limit reached. Retrying in ${backoffMs / 1000} seconds...]\n`;
      ...
      if (error?.status === 400 || error?.message?.includes("400")) {
        ...
        throw new Error(`Bad Request (400): ${errorMessage}. Please check your API key and request format.`);
      }
      
      if (isRateLimitError(error)) {
        throw new Error(`Rate limit exceeded (429). Please wait a moment before sending another message. If this persists, you may need to check your API quota or upgrade your plan.`);
      }
  ...
  if (lastError && isRateLimitError(lastError)) {
    throw new Error(`Rate limit exceeded after ${maxRetries + 1} attempts. Please wait a few minutes before trying again.`);
  }
